...
public class ServiceImpl implements Service {
    ...
    public ServiceImpl(OrderDao myOrderDao, ProductDao myProductDao, TaxesDao myTaxesDao, IdDao myIdDao) {
    ...
    public List<Order> getAllOrders(LocalDate date) throws InvalidDateException, OrderNotFoundException {
        if (myOrderDao.readByDate(date) == null) {
            throw new OrderNotFoundException("Order with that criteria.");
        }
    ...
    public Order addAnOrder(Order order) throws PersistenceException, InvalidProductException, InvalidStateException, InvalidNameException {
        ...
        myOrderDao.create(LocalDate.now(), order);
        return order;
    ...
    public Order getOrder(LocalDate date, int orderNum) throws PersistenceException, OrderNotFoundException {
        if (myOrderDao.readById(date, orderNum) == null) {
            throw new OrderNotFoundException("No order exists with that criteria.");
        }
    ...
    public boolean deleteOrder(Order order, LocalDate date) throws InvalidDateException, PersistenceException {
        if (myOrderDao.readById(date, order.getOrderNumber()) == null) {
            throw new InvalidDateException("No Order with that criteria found.");
        }
    ...
    @Override
    public void saveOrder(boolean saveOrder) {
        if (saveOrder) {
            myOrderDao.save();
